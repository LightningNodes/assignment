...
import bg from "../../images/PriyaBit.jpeg";
    ...
    <div className={styles.container}>
      <div className={styles.navbar}>
        <div className={styles.logo}>
          <img
            src={bg}
            style={{ width: "90px", height: "50px", borderRadius: "8px" }}
          />
        </div>
      ...
      <h1 className={`${styles.heading} `}>
        Deal in Crypto Perpetual Futures using INR
      </h1>
      <p className={`${styles.message} ${styles.neonEffect}`}>
        {currentUser
          ? `Thank you for coming back, ${currentUser?.displayName}!`
...
function generateInitialData() {
  ...
  for (let i = 0; i < 60; i++) {
    // Change from 10 to 30 for one month data
    data.push({
      x: new Date(time),
      y: [
